...
class Agent:
    ...
    def goal_reached(self):
        return (self.goal_x == self.pos_x) and (self.goal_y == self.pos_y) 
    ...
    def run(self):
        ...
        while(not self.goal_reached() and steps < 100):
            counter = 0
            direction = -1
            while(True): # if all moves in any direction are not valid, the agent just stays in the same place
                direction = self.activate_net(self._get_map_env())
                valid = self.move(direction)                              
